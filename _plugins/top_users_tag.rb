...
module Jekyll
  class TopUsersTag < Liquid::Tag
    ...
    def getEachUserData(users)
      p "Getting data for each #{users["items"]} users"
      top_users = []
      users["items"].each do |user|
        p "Getting data for user #{user["login"]}"
    ...
    def getTopUsersData()
      if File.exist?("top_users_data.json")
        ...
        if File.exist?("top_friends.json")
          ...
          data["users"] = data["users"].map { |u| { "login" => u } }
          users = { "items" => data["users"] }
          @top_users = getEachUserData(users)
        else
